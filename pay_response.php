...
function regenarate_session($uid){

      $user_q=select("SELECT * FROM `user_info` WHERE `id`=? LIMIT 1",[$uid],'i');
      $user_fetch=mysqli_fetch_assoc($user_q);

      $_SESSION['login']=true;
      $_SESSION['uid'] = $user_fetch['id'];
      $_SESSION['uname'] = $user_fetch['name'];
      $_SESSION['upic'] = $user_fetch['picture'];
...
if($isValidChecksum == "TRUE") {
   ...
   {
      regenarate_session($slct_fetch['user_id']);
   }
	...
	if ($_POST["STATUS"] == "TXN_SUCCESS") 
   {
      $upd_query="UPDATE `booking_order` SET `booking_status`='booked',
       `trans_id`='$_POST[TXNID]',`trans_amount`='$_POST[TXNAMOUNT]',`trans_status`='$_POST[STATUS]',
