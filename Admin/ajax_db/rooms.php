<?php
    ...
    if(isset($_POST['get_all_rooms'])){
      ...
      while($row=mysqli_fetch_assoc($res)){
        ...
        if($row['status']==1){
          ...
          $status="<button onclick='toggle_status($row[id],1)' class='btn btn-warning'>inactive</button>";
        }
      ...
      <tr class='align-middle'>
       ...
       <td>$status</td>
       <td>    
       <button type='button' onclick=\"room_images($row[id],'$row[name]')\" class='btn btn-info text-light btn-sm' data-bs-toggle='modal' data-bs-target='#room_images'>
       <i class='bi bi-images'></i>
       </button>       
       <button type='button' onclick='remove_room($row[id])' class='btn btn-danger text-light btn-sm'>
        ...
        if($row['thumb']==1)
         {
          ...
          $thumb_btn="<button onclick='thumb_image($row[sr_no],$row[room_id])' class='btn btn-secondary text-light btn-sm'>
         ...
         echo<<<data
         <tr class='align-middle'>
         <td><img src='$path$row[image]' width='200px' class='image-fluid'></td>
         <td>$thumb_btn</td>
         <td>
         <button type='button' onclick='rem_image($row[sr_no],$row[room_id])' class='btn btn-danger text-light btn-sm'>
         <i class='bi bi-trash'></i>
         </button> 
    ...
    if(isset($_POST['thumb_image']))
    {
      ...
      $preq="UPDATE `room_images` SET`thumb`=? WHERE `room_id`=?";
      $prev=[0,$frm_data['room_id']];
